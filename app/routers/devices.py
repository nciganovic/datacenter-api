...
from app.models import DeviceForm, Device, Rack
...
def get_single(device_id: int, session: Session = Depends(get_session)):
...
def device_validation(device: Device, session: Session, create_mod: bool):
    ...
    if(device.rack_id is not None):
        ...
        if(rack is None):
            raise HTTPException(
                status_code=400,
                detail=f"Rack with id {device.rack_id} does not exist."
            )
        ...
        if(total_consumption + device.power_consumption > rack.max_power_consumption):
            raise HTTPException(
                status_code=400,
                detail=f"Power consumption exceeds maximum allowed value."
            )
        
        #same here but for unit size
        total_unit_size: int
        if(create_mod):
